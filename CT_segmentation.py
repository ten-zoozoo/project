...
def render_slice(ct_vol: np.ndarray, z_pred: int, wl: float, ww: float, alpha: float,
    ...
    if mask_path is None:
        ...
        mv = np.asanyarray(nib.load(str(mask_path), mmap=False).dataobj)
...
def _get_pred_zmax(p: Path | None) -> int | None:
    if p is None:
        return None
    try:
        mv = np.asanyarray(nib.load(str(p),mmap=False).dataobj)
        if mv.ndim == 3:
...
if ct_file is not None:
    ...
    ct_img = nib.load(str(ct_nii_path), mmap=False)
